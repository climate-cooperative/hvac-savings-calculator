const states = [
  ...
  const convertZipToState = (zipcode: number) => {
  ...
  const getZip = async (zipcode: number) => {
  ...
  const getHvacAppliances = async (heating_appliance: string) => {
    const heating_url = `${BASE_URL}/hvac/${encodeURIComponent(heating_appliance)}`;
    const response = await fetch(heating_url);
    const data = await response.json();
  
    const hvac_efficiency = data[0]["efficiency"];
